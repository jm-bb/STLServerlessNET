    ...
    public string GetAttributeDetails(string sku, string webAttributes)
    {
        try
        {
            if (webAttributes.Trim().Length > 0)
            {
                ...
                string[] attributes = webAttributes.Split(new string[] { ";" }, StringSplitOptions.RemoveEmptyEntries);
                foreach (string attribute in attributes)
                {
                    string[] attribValues = attribute.Split(new string[] { "|" }, StringSplitOptions.RemoveEmptyEntries);
